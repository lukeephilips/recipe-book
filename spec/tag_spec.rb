...
describe(Tag) do
  ...
  describe('#recipes.ingredients') do
    it('adds a recipe with a join to this tag') do
      recp = Recipe.create(:title => 'pasta salad', :cook_time => '00:30:00', :servings => 5, :spice => 0, :rating => 1)
  ...
  describe('#save') do
    it('validates that the tag name is unique') do
      test_tag = Tag.new(:name => 'Vegetarian', :category => 'Diet')
